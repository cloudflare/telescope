...
describe.each(browsers)('Device Emulation CLI + Config tests using device properties', browser => {
  describe.each(target_devices)('Setting device emulation updates the config for device: %s',
    device => {
      let options = {
        browser,
        deviceName: device,
      ...
      let config = new BrowserConfig().getBrowserConfig(browser, config_options);
...
describe.each(browsers)('Device Emulation CLI + Config tests with width + height overrides', browser => {
  describe.each(target_devices)('Setting device emulation updates the config for device: %s',
    device => {
      let options = {
        browser,
  ...
  describe.each(target_devices)(
    ...
    device => {
      ...
      test(`launchTest resultsPath exists on disk for browser: ${browser}`, () => {
        expect(fs.existsSync((result as SuccessfulTestResult).resultsPath)).toBe(true);
      })
    },
  );
});
