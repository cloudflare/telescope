...
export function normalizeCLIConfig(options) {
  ...
  if (options.device && options.device.length > 0) {
    ...
    if (!device) {
      throw new Error(`Device "${options.device}" not found in Playwright device list`);
    }
  ...
  } else if (device.viewport && device.viewport.width) {
    config.width = device.viewport.width;
  } else {
  ...
  } else if (device.viewport && device.viewport.height) {
    config.height = device.viewport.height;
  } else {
