...
export function normalizeCLIConfig(options) {
  ...
  if (options.block) {
    ...
    options.block.forEach((opt_group) => {
      if (opt_group.includes('[')) { // Looks like a JSON array
        try {
          const json_obj = JSON.parse(opt_group);
          json_obj.forEach((blockMe) => {
            config.block.push(blockMe);
          });
        } catch (err) {
          console.error(err);
        }
      } else {
        opt_group.split(/\s*,\s*/).forEach((opt) => {
          if (opt) {
            config.block.push(opt);
          }
        });
      }
    });
