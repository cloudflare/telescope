...
class TestRunner {
  ...
  /**
   * Set up response first byte delays using the page.route handler
   */
  async setupFirstByteDelays(page) {
    const DELAY_METHOD_FALLBACK = 'fallback';
    const DELAY_METHOD_CONTINUE = 'continue';
    const DELAY_METHOD = DELAY_METHOD_FALLBACK;

    Promise.all(
      Object.entries(this.options.delayFirstByte).map(
        ([regexString, delayFirstByte]) => {
          log(
            `Adding a rule for delaying URLs matching '${regexString}' for ${delayFirstByte}`,
          );
          ...
          return page.route(
            ...
            route =>
              new Promise((resolve, reject) => {
                if (DELAY_METHOD === DELAY_METHOD_FALLBACK) {
                  // start fetching right-away, keep the promise
                  const responsePromise = route.fetch();

                  log(
                    `Fetching ${url} (${regex}), but delaying response for ${delayFirstByte}ms`,
                  );

                  setTimeout(async () => {
                    log(`Fulfilling ${url} after ${delayFirstByte}ms`);
                ...
                if (DELAY_METHOD === DELAY_METHOD_CONTINUE) {
                  log(
                    `Delaying ${url} (${regex}) request for ${delayFirstByte}ms`,
                  );

                  setTimeout(() => {
                    log(`Continuing ${url} request after ${delayFirstByte}ms`);
  ...
  async preparePage(page) {
    ...
    await this.setupFirstByteDelays(page);
