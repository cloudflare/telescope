...
class TestRunner {
  ...
  /**
   * Set up any request delays using the page.route handler
   */
  async setupRequestDelays(page) {
    await page.route('**/*', async route => {
      // this runs on each request
      let bypassDelay = true;

      for (const [regex, delay] of Object.entries(this.options.delayRequests)) {
        const url = route.request().url();
        if (url.match(regex) !== null) {
          console.log(`Delaying ${url} (${regex}) for ${delay}ms`);
          bypassDelay = false;
          ...
          setTimeout(async () => {
            console.log(`Fulfilling ${url} after ${delay}ms`);
  ...
  async preparePage(page) {
    ...
    await this.setupRequestDelays(page);
