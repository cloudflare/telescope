...
import { DELAY_IMPLEMENTATIONS } from './delay.js';
...
class TestRunner {
   ...
   */
  async setupResponseDelays(page) {
    Promise.all(
      Object.entries(this.options.delay).map(async ([regexString, delay]) => {
        ...
        let delayImplementation =
          DELAY_IMPLEMENTATIONS[this.options.delayUsing];

        return await page.route(regex, async (route, request) =>
          delayImplementation(route, request, regexString, delay),
        );
      }),
    );
  }
