...
class TestRunner {
  ...
  /**
   * Setup the everything route.
   * This is registered first, but run last
   */
  async setupRoute(page) {
  ...
  async setupHostOverrides(page, overrides) {
    ...
    await page.route(domain_rx, async (route, request) => {
      ...
      const original_domain = parts.findLast((d) => { return !!d; }); /* Grab what matched */
      const host_rx = new RegExp('//' + original_domain);
      const new_url = original_url.replace(host_rx, '//' + overrides[original_domain]);
