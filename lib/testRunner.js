...
class TestRunner {
  ...
  async setupFirstByteDelays(page) {
    await page.route('**/*', async route => {
      for (const [regex, delayFirstByte] of Object.entries(
        this.options.delayFirstByte,
      )) {
        ...
        if (url.match(regex) !== null) {
          ...
          setTimeout(async () => {
            console.log(`Fulfilling ${url} after ${delayFirstByte}ms`);
          ...
          // match the first regex, ignore the rest of the delay rules
          return;
        }
      }

      // if no rules matched, just continue
      route.continue();
    });
  }
