...
class TestRunner {
  ...
  async postProcess() {
    ...
    if (this.options.uploadUrl) {
      try {
        let outputZip = `./results/${this.TESTID}.zip`;
        // if file hasn't been zipped yet, do it now but in temporary context so it is cleaned up later
        if (!existsSync(outputZip)) {
          outputZip = `./${this.paths['temporaryContext']}/${this.TESTID}.zip`;
          const zip = new AdmZip();
          zip.addLocalFolder(this.paths['results']);
          zip.writeZip(outputZip);
        }
        ...
        formData.append('file', createReadStream(outputZip));
        // config validation done earlier so we can use uploadUrl value directly
        await fetch(this.options.uploadUrl, {
          method: 'POST',
          body: formData,
        });
      } catch (err) {
        console.error(`Error uploading zip file to ${this.options.uploadUrl}:`, err);
      }
    }
