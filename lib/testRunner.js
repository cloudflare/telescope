...
class TestRunner {
  ...
  async postProcess() {
    ...
    if (this.options.html) {
      ...
      try {
        copyFileSync(url.fileURLToPath(new URL(
          `../img/${this.selectedBrowser.channel || this.selectedBrowser.engine}.png`,
          import.meta.url
        )), this.paths['results'] + '/engine.png');

        const testTemplate = readFileSync(url.fileURLToPath(new URL(
          './templates/test.ejs',
          import.meta.url
        )), 'utf8').toString();
    ...
    if (this.options.list) {
      ...
      const listTemplate = readFileSync(url.fileURLToPath(new URL(
        './templates/list.ejs',
        import.meta.url
      )), 'utf8').toString();
