  ...
  async postProcess() {
    ...
    if (this.options.html) {
      ...
      copyFileSync(
        path.resolve(
          currentDir,
          `../img/${this.selectedBrowser.channel || this.selectedBrowser.engine
          }.png`,
        ),
    ...
    const outputZip = path.resolve('./results', `${this.TESTID}.zip`);
    const zip = (path, zipfile) => {
      try {
    ...
    // handle the zipping
    if (this.options.zip) {
      zip(this.paths['results'], outputZip);
    }

    // handle uploading
    if (this.options.uploadUrl) {
      try {
        const zipped = existsSync(outputZip);
        // if file hasn't been zipped yet, do it now
        if (!zipped) {
          zipped = zip(this.paths['results'], outputZip);
        }

        if (zipped) {
          const formData = new FormData();
          const fileBuffer = await readFile(outputZip);
          formData.append('file', new Blob([fileBuffer]), {
            filename: `${this.TESTID}.zip`,
          });
        ...
        console.error(
          `Error uploading zip file to ${this.options.uploadUrl}:`,
          err,
        );
      } finally {
        if (!this.options.zip && existsSync(outputZip)) {
          rmSync(outputZip);
        }
      }
    }
