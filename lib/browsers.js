...
class BrowserConfig {
  ...
  getBrowserConfig(browser, options) {
    ...
    if (options.mobile && options.mobile.length > 0) {
      const device = devices.devices[options.mobile];
      if (!device) {
        throw new Error(`Mobile device "${options.mobile}" not found in Playwright device list`);
      }
      Object.assign(browserConfig, device)
      // manually override video size to match device viewport -- not part of playwright device object
      browserConfig.recordVideo.size = device.viewport;
    }
